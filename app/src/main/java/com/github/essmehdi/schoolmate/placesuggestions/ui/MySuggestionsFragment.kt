...
import com.github.essmehdi.schoolmate.databinding.FragmentMySuggestionsBinding
...
class MySuggestionsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentMySuggestionsBinding.inflate(inflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.showMineEmpty.observe(viewLifecycleOwner) {
            showEmpty(it)
        }

        // Update adapter on data change
        viewModel.mySuggestions.observe(viewLifecycleOwner) { suggestionsList ->
            suggestionsList?.let { suggestionAdapter.updateData(it) }
            Log.d("Suggestions MINE", "MINE: $suggestionsList")
        }

        viewModel.myCurrentPageStatus.observe(viewLifecycleOwner) { currentPage ->
            when (currentPage) {
